...
function createKafka(config) {
  // retry option retries up to retry.retries times with each retry roughly
  // doubling in time between calls. Max time of
  // retry.initialRetryTime/1000 * (2 ^ retries) seconds
  const retry = {
...
function loadConfigFromEnv() {
  ...
  const acks = config.fromEnvironment('KAFKA_PRODUCER_ACKS', -1);
...
function assertConfigValid(config) {
  ...
  if (isNaN(config.acks)) {
    throw new Error('Acks must be a number');
  }
  if (config.acks < -1 || config.acks > 1) {
    throw new Error('Acks must be between -1 and 1');
  }
